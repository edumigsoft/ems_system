...
///
/// Utiliza [NotebookFormViewModel] para gerenciamento de estado e validação.
...
class _NotebookFormPageState extends State<NotebookFormPage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (validation case Failure()) {
      // Erros já estão mapeados no ViewModel
      return;
    }

    if (!mounted) return;

    // Dados validados - criar DTO e chamar callback
        ...
        builder: (context, _) {
          return ListView(
            ...
            children: [
              // Título
              TextField(
                controller: _viewModel.registerField(notebookTitleField),
                decoration: InputDecoration(
                  labelText: 'Título *',
                  hintText: 'Digite o título do caderno',
                  border: const OutlineInputBorder(),
                  errorText: _viewModel.getFieldError(notebookTitleField),
                ),
                maxLength: 255,
              ),
              ...
              // Conteúdo
              TextField(
                controller: _viewModel.registerField(notebookContentField),
                decoration: InputDecoration(
                  labelText: 'Conteúdo *',
                  hintText: 'Digite o conteúdo ou resumo',
                  border: const OutlineInputBorder(),
                  alignLabelWithHint: true,
                  errorText: _viewModel.getFieldError(notebookContentField),
                ),
                maxLines: 10,
                minLines: 5,
              ),
                ...
                children: [
                  ...
                  ElevatedButton(
                    ...
                    child: _viewModel.isSubmitting
                        ...
                        : Text(_viewModel.isEditing ? 'Salvar' : 'Criar'),
                  ),
                ],
              ),
            ],
