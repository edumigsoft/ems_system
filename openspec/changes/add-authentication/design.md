...
### 5. Email Service para Reset de Senha

**Decisão**: Preparar para receber configuração via variáveis de ambiente.
...
**Rationale**:
- Flexibilidade para diferentes providers (SendGrid, Mailgun, SMTP direto)
- Configuração por ambiente sem mudança de código
- Interface abstrata `EmailService` para implementações futuras
...
1. **Fase 1**: Usar `create_feature_wizard.sh` para scaffold
2. **Fase 2**: Implementar pacote `auth` completo
3. **Fase 3**: Registrar `InitAuthModuleToServer` em `injector.dart`
4. **Fase 4**: Integrar em app cliente
5. **Rollback**: Comentar registro no injector, rotas são independentes

## Open Questions

- [x] ~~Qual email service usar para reset de senha?~~ → Configurável via env
