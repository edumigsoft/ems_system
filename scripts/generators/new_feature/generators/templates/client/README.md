...
Pacote client da feature **{{FEATURE_TITLE}}**. Implementação HTTP usando Dio/Retrofit.
...
```dart
import 'package:{{FEATURE_NAME}}_client/{{FEATURE_NAME}}_client.dart';

// Configurar Dio
final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));

// Criar repository
final repository = {{ENTITY_NAME}}RepositoryClient(dio);

// Usar com use cases
final useCase = Get{{ENTITY_NAME}}sUseCase(repository);
